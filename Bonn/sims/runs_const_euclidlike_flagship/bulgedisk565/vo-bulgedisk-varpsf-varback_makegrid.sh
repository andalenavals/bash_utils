...
WORKDIR=/vol/euclidraid4/data/aanavarroa/catalogs/MomentsML/fullimages_constimg_euclid-flagship/vo-bulgedisk-varpsf-varback-grid-big
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --transformtogriddir=$BLENDEDDIR --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --psffilesdir=$PSFFILESDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --tru_type=2 --const_type=sc8 --pixel_conv --dist_type=flagship --cattype=tru --ncpu=100 --ncat=200 --typegroup=tw --skipdone --substractsky --runadamom --match_pairs --rot_pair #--transformtogrid --runsims --usepsfimg --usevarpsf --usevarsky #--use_weight --runsex --run_check
