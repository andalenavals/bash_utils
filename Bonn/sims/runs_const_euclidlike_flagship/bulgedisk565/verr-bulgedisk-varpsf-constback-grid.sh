...
WORKDIR=/vol/euclidraid4/data/aanavarroa/catalogs/MomentsML/fullimages_constimg_euclid-flagship/verr-bulgedisk-varpsf-constback-grid
SIMDIR=$WORKDIR/sim
SEXDIR=$WORKDIR/sex
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --psffilesdir=$PSFFILESDIR --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SCRIPTDIR/configfiles_const/oldsexconf.yaml --tru_type=2 --const_type=sc8 --pixel_conv --substractsky --dist_type=uniflagship --cattype=tru --ncpu=100 --ncat=10000 --skipdone --typegroup=tp --runsims --usepsfimg --usevarpsf --runadamom #--savetrugalimg #--adamom_weight --runsex
