...
WORKDIR=/vol/euclidraid4/data/aanavarroa/catalogs/MomentsML/fullimages_constimg_euclid-flagship/vp-bulgedisk-varpsf-varback-grid
SIMDIR=$WORKDIR/sim
SEXDIR=$WORKDIR/sex
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --psffilesdir=$PSFFILESDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SCRIPTDIR/configfiles_const/oldsexconf.yaml --tru_type=2 --const_type=sc8 --pixel_conv --substractsky --dist_type=uniflagship --cattype=sex --ncpu=250 --ncat=5000 --skipdone --typegroup=tp --runadamom --runsex #--runsims --usepsfimg --usevarsky --usevarpsf #--savetrugalimg #--adamom_weight --runsex 
