...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR  --ksbdir=$KSBDIR --psffilesdir=$PSFFILESDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --tru_type=2 --const_type=sc8 --pixel_conv --dist_type=flagship --cattype=sex --ncpu=20 --ncat=200 --typegroup=tw --skipdone --substractsky --runsims --usepsfimg --usevarsky --usevarpsf --rot_pair #--runadamom --use_weight --aperture_r=20  --runneis #--runsex --run_check
