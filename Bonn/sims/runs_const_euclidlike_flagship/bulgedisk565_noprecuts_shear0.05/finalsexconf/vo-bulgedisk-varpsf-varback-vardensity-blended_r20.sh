...
WORKDIR=/vol/euclidraid4/data/aanavarroa/catalogs/MomentsML/fullimages_constimg_euclid-flagship0.05/vo-bulgedisk-varpsf-varback-blended-vardensity-big
SIMDIR=$WORKDIR/sim
...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR  --ksbdir=$KSBDIR --psffilesdir=$PSFFILESDIR --adamompsfcatalog=$ADAMOMPSFCAT --groupcats=$GROUPCATS --cat_args=$CAT_ARGS --sex_args=$SEX_ARGS --tru_type=2 --const_type=sc8 --pixel_conv --dist_type=flagship --cattype=sex --ncpu=250 --ncat=200 --typegroup=tw --skipdone --substractsky --runadamom --use_weight --aperture_r=20  --runneis --rot_pair #--runsex --run_check --runsims --usepsfimg --usevarsky --usevarpsf
