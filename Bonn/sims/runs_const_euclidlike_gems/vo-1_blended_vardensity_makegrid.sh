...
python run_sim_meas_constimg_euclidlike.py --loglevel=INFO --transformtogriddir=$BLENDEDDIR --simdir=$SIMDIR --sexdir=$SEXDIR --adamomdir=$ADAMOMDIR --ksbdir=$KSBDIR --groupcats=$GROUPCATS --cat_args=$SCRIPTDIR/configfiles_const/tw-big.yaml --sex_args=$SCRIPTDIR/configfiles_const/oldsexconf.yaml --dist_type=gems --cattype=tru --ncpu=100 --ncat=200 --typegroup=tw --skipdone --runadamom --rot_pair --substractsky --matchinput --match_pairs  #--runadamom --runsex #--transformtogrid --runsims --usepsfimg --runadamom --rot_pair #--use_weight --runneis --runsex --match_pairs --matchinput
